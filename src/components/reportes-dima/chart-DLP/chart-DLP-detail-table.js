...
import { DLPDetailTable } from '../../../services/reportes-dima';
...
export default function CollapseDLPDetailTable({ date, ...rest }) {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const res = await DLPDetailTable({ year: date.year, month: date.month, order: order, orderBy: orderBy })
                setResults(res.data)
            } catch (e) {
                console.log(e)
            }
