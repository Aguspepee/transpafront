...
import { DTNDetailTable } from '../../../services/reportes-dima';
...
export default function CollapseDTNDetailTable({ date, ...rest }) {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const res = await DTNDetailTable({ year: date.year, month: date.month, order: order, orderBy: orderBy })
                setResults(res.data || [])
            } catch (e) {
                console.log(e)
            }
