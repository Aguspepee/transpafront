...
import { DLFDetailTable } from '../../../services/reportes-dima';
...
export default function CollapseDLFDetailTable({ date, ...rest }) {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const res = await DLFDetailTable({ year: date.year, month: date.month, order: order, orderBy: orderBy })
                setResults(res.data)
            } catch (e) {
                console.log(e)
            }
